...
export const WebhookManager: React.FC = () => {
  ...
  const testIncomingWebhook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setError('Incoming webhook test executed successfully');
        // Refresh messages
        await loadMessages(id);
      } else {
        setError(`Webhook test failed: ${result.message}`);
      }
    } catch (err) {
      console.error('Error testing incoming webhook:', err);
      setError('Failed to test incoming webhook');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        <h3 className="text-xl font-semibold mb-4">Test Incoming Webhook</h3>
        <p className="text-gray-600 mb-4">
          Use this form to quickly test the incoming webhook functionality by sending a test message to this bot.
          Messages sent here will appear in the "Recent Incoming Messages" section below.
        </p>
        <form onSubmit={testIncomingWebhook} className="space-y-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Optional JSON data to include with the webhook
            </p>
          </div>
